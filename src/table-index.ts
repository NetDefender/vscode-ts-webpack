...
function UpdateWithNewCustomId() {
  document
    .querySelectorAll(".container-top *")
    .forEach((element, index) =>
      element.setAttribute("data-uid", index.toString())
    );
}
...
function UpdateRanges(titleList: TitleNode[]) {
  ...
  workTitles.forEach((item, index) => {
    if (index !== 0) {
      ...
      range.setStart(
        document.querySelector(`[data-title-id="${prevTitle.NodeId}"]`)!,
        0
      );
      const endElement = document.querySelector(
        `[data-title-id="${currentTitle.NodeId}"]`
      )!;
        ...
        .forEach((element, index) => {
          ...
          if (subElement) {
            ...
            subElement.setAttribute(
              "data-scroll-id",
              targetNodeId === currentTitle.NodeId
                ? currentTitle.NodeId
                : prevTitle.NodeId
            );
...
let isLoading = true;
const observer = new IntersectionObserver(TitlesIntersected, {
  root: document.getElementById("container-top"),
});
...
function TitlesIntersected(
  ...
  entries.forEach(
    ({
      ...
      const titleSpan = titleLi.querySelector(
        ".link:first-child"
      )! as HTMLSpanElement;
