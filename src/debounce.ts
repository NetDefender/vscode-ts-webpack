...
function debounceHTMLEvent<K extends keyof HTMLElementEventMap>(type: K, action: (() => void), timeout: number = 500) {
...
function searchAnimals() {
  setView(animals.filter(animal => animal.name.toLowerCase().includes(search.value?.toLocaleLowerCase())));
}
...
function render() {
  ...
  function renderInternal() {
    let rows: string = view.map(animal => `<tr data-id="${animal.id}"><td>${animal.name}</td></tr>`).join('');
