...
class RpcFrame {
  ...
  public wait(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.iframe!.addEventListener('load', () => this.frameLoaded(resolve, reject))
    })
  }

  private frameLoaded(resolve: (value: boolean | PromiseLike<boolean>) => void, reject: (reason?: any) => void) {
    this.iframe!.removeEventListener('load', () => this.frameLoaded(resolve, reject));
...
class RpcClient {
  ...
  connect(): RpcClient {
    ...
    this.channel.port1.onmessage = (event: MessageEvent<any>) => this.response(event);
  ...
  call(message: RpcMessage) : Promise<RpcResult<any>> {
    ...
    if(!this.sendPort) {
      ...
      this.frame.frame?.contentWindow?.postMessage(message, '*', [this.channel.port2]);
    } else {
...
class RpcServer {
  ...
  private methods: Map<RpcMethod, RpcMethodCallSignature> = new Map<RpcMethod, RpcMethodCallSignature>();
  ...
  registerMethods(): RpcServer {
    this.methods.set('getUser', (message) => this.wrapResult(message, () => new UserService().getUser(new Number(message.parameters['idUser']).valueOf())))
    this.methods.set('deleteUser', (message) => this.wrapResult<void>(message, () => new UserService().deleteUser(new Number(message.parameters['idUser']).valueOf())))
    return this;
  }
...
async function bootStrap() {
  ...
  const contextValue: any = document.querySelector('meta[name="context"]')?.getAttribute('content')?.valueOf();
  if(contextValue) {
